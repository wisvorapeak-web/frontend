...
export default function AdminOverview() {
  ...
  if (loading) return <AdminLayout><div className="text-[10px] font-black uppercase tracking-widest text-slate-400 p-12">Initializing Console...</div></AdminLayout>;
    ...
    <AdminLayout>
      <div className="space-y-16">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 pb-8 border-b border-slate-100">
          <div className="space-y-4">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-blue/5 border border-blue/10 rounded-full">
              ...
              <span className="text-[9px] font-black text-blue uppercase tracking-widest">Real-time Metrics</span>
            </div>
            <h1 className="text-4xl font-black text-navy uppercase tracking-tighter leading-none">Console <span className="text-blue">Manifesto</span></h1>
          </div>
          <div className="flex items-center gap-4 p-4 border border-slate-100 rounded-2xl">
            <Clock className="w-4 h-4 text-slate-300" />
            <div>
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none">Last Synchronization</p>
              <p className="text-[11px] font-black text-navy uppercase tracking-widest mt-1">Recently Active</p>
            </div>
          </div>
        </div>
          ...
          {displayStats.map((stat) => (
            <div key={stat.label} className="p-8 border border-slate-100 rounded-2xl hover:border-blue/20 hover:shadow-2xl hover:shadow-blue/5 transition-all space-y-6">
              <div className="flex justify-between items-center">
                ...
                <span className="text-[9px] font-black text-blue px-2 py-0.5 bg-blue/5 rounded-full uppercase tracking-widest">{stat.change}</span>
              </div>
              <div className="space-y-1">
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{stat.label}</p>
                <h3 className="text-3xl font-black text-navy uppercase tracking-tighter">{stat.value}</h3>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 p-10 border border-slate-100 rounded-3xl space-y-10">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-black text-navy uppercase tracking-tight">System <span className="text-blue">Transmissions</span></h3>
              <div className="flex gap-2">
                {['D', 'W', 'M'].map(t => (
                  <button key={t} className={`w-8 h-8 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${t === 'W' ? 'bg-blue text-white' : 'text-slate-400 hover:bg-slate-50'}`}>{t}</button>
                ))}
              </div>
            </div>
            <div className="h-64 flex items-end gap-3 pt-6">
              {adminStats.chartData.map((val, i) => (
                <div key={i} className="flex-1 group relative h-full flex flex-col justify-end">
                  ...
                  <p className="mt-4 text-[8px] font-black text-slate-300 uppercase tracking-tighter text-center">CYCLE {i+1}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="p-10 bg-navy rounded-3xl space-y-10 relative overflow-hidden">
            ...
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-lg font-black text-white uppercase tracking-tight">Direct <span className="text-blue">Inbox</span></h3>
            </div>
            ...
            <div className="space-y-4">
              {adminStats.recentInquiries.length > 0 ? adminStats.recentInquiries.map((inquiry: any) => (
                <div key={inquiry.id} className="p-6 bg-white/5 border border-white/5 rounded-2xl space-y-3 hover:bg-white/10 transition-all cursor-pointer">
                  <p className="text-[11px] font-black text-white uppercase tracking-widest truncate">{inquiry.name}</p>
                  <p className="text-[10px] font-bold text-white/30 uppercase tracking-widest leading-relaxed line-clamp-1">{inquiry.subject}</p>
                </div>
              )) : (
                <div className="py-12 text-center space-y-4">
                  ...
                  <p className="text-[10px] font-black text-white/20 uppercase tracking-[0.2em]">Queue Clear</p>
                </div>
              )}
            </div>
            
            <button className="w-full h-14 bg-blue text-white text-[10px] font-black uppercase tracking-[0.2em] rounded-2xl hover:bg-white hover:text-navy transition-all">
              Initialize Inbox
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            { label: 'Personnel Console', sub: 'Manage Scientists', icon: Users, color: 'text-blue', path: '/admin/users' },
            { label: 'Scientific Board', sub: 'Review Submissions', icon: CheckCircle2, color: 'text-indigo-600', path: '/admin/abstracts' },
            { label: 'Financial Vault', sub: 'Transaction Ledger', icon: CreditCard, color: 'text-emerald-600', path: '/admin/registrations' },
          ].map((action, i) => (
            <button 
              key={i} 
            ...
            >
              ...
              <div className="space-y-1">
                <h4 className="text-sm font-black text-navy uppercase tracking-tight">{action.label}</h4>
